...
public:
    ...
    */
    copy_on_write(T x) :
        object_m(allocate_move(0, boost::move(x)))
    { }
    ...
    */
    ...
    copy_on_write& operator=(T x)
    {
        if (!object_m)
            ...
            object_m->value_m = boost::move(x);
        else
            reset(allocate_move(object_m, boost::move(x)));
...
private:
    ...
    static implementation_t* allocate_move(const implementation_t* alloc_src, T x)
    {
        ...
        try {
            ::new(static_cast<void*>(tmp)) implementation_t(boost::move(x));
        } catch (...) {
            tmp->get_allocator().deallocate(tmp, 1);
            throw;
        }
