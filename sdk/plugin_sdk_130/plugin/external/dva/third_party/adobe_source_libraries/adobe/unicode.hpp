...
/**************************************************************************************************/
	...
	template <typename InputIterator, typename T>
	InputIterator to_utf32 (InputIterator first, InputIterator last, T& result, unicode_size_type_<1>, bool* isUTFCompliant = NULL)
	{
		...
		if (n < to_utf32_pivot_1_k) {
			...
			if (n < to_utf32_pivot_2_k) {
				//ADOBE_ASSERT(false && "unicode: ill-defined UTF-8 (first byte is 10xxxxxx)");
				if (isUTFCompliant) {
					*isUTFCompliant = false;
				}
			}
		...
		if (result > 0x0010FFFF) {
			//ADOBE_ASSERT(false && "unicode: ill-defined UTF-8 (code point out of range)");
			if (isUTFCompliant) {
				*isUTFCompliant = false;
			}
		}
		...
		if (0x0000D800 <= result && result <= 0x0000DFFF) {
			//ADOBE_ASSERT(false && "unicode: ill-defined UTF-8 (surrogate code point)");
			if (isUTFCompliant) {
				*isUTFCompliant = false;
			}
		}
...
template <  typename T,
            typename I, // I models InputIterator
            typename O> // O models OutputIterator
	O to_utf16(I first, I last, O output, unicode_size_type_<1>, bool* isUTFCompliant = NULL)
	{
		while (first != last)
		{
			boost::uint32_t result;
			
			first = to_utf32(first, last, result, unicode_size_type_<1>(), isUTFCompliant);
			
			if (isUTFCompliant && !(*isUTFCompliant))
				break;
