...
#define ID_NOEXCEPT_SPEC
#else // Windows or libc++
#define ID_NOEXCEPT_SPEC BOOST_NOEXCEPT
#endif
...
/*
    ...
    Disabling these and moving them to under eve.cpp, so we can better control
    the formatting (iomanip is, unfortunately, pretty broken, as everything gets
    pumped through an any_regular_t which only handles a small set of types for
    serialization.)
*/
#if !defined(NDEBUG) && defined(ADOBE_STD_SERIALIZATION) && 0
...
template <> inline void swap(adobe::extents_t::slice_t& x, adobe::extents_t::slice_t& y) ID_NOEXCEPT_SPEC
{
...
template <> inline void swap(adobe::extents_t& x, adobe::extents_t& y) ID_NOEXCEPT_SPEC
