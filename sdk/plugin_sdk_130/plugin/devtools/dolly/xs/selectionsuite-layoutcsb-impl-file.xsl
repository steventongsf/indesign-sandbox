...
<xsl:template match="impl-item[@impl-file = 'selectionsuite-layoutcsb-impl-file']">
...
<xsl:call-template name="standard-adobe-header"/>
  <xsl:call-template name="selectionsuite-layoutcsb"/>
  <xsl:call-template name="standard-footer"/>
 </xsl:document>
</xsl:template>

<xsl:template name="selectionsuite-layoutcsb" match="selectionsuite-layoutcsb">
// Interface includes:
...
/** <xsl:value-of select="@impl-name"/> is the layout specific implementation of <xsl:value-of select="/code/@short-name"/>SuiteCSB.
