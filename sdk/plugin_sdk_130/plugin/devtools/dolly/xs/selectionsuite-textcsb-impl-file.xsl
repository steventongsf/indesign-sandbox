...
<xsl:template match="impl-item[@impl-file = 'selectionsuite-textcsb-impl-file']">
...
<xsl:call-template name="standard-adobe-header"/>
  <xsl:call-template name="selectionsuite-textcsb"/>
  <xsl:call-template name="standard-footer"/>
 </xsl:document>
</xsl:template>

<xsl:template name="selectionsuite-textcsb" match="selectionsuite-textcsb">
// Interface includes:
...
/** <xsl:value-of select="@impl-name"/> is the text specific implementation of <xsl:value-of select="/code/@short-name"/>SuiteCSB.
